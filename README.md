...
The tenant and user schema enforces globally unique tenant slugs and unique `(tenant_id, email)` pairs, plus the user-to-tenant foreign key and status/role enums. A user belongs to one tenant. `users` is the sole identity and credential source for every account role. Sessions reference a user and contain only a hash of the random cookie token; roles and tenant membership are read from that user record when requests are authenticated. Signup creates tenant, owner user, and session in one transaction. Use the database's normal transaction isolation unless a demonstrated invariant needs stronger isolation.

Authentication routes are `/api/v1/auth/register`, `/api/v1/auth/login`, `/api/v1/auth/me`, and `/api/v1/auth/logout`. Signup creates the initial workspace owner. Login uses the workspace slug plus email and password because email uniqueness is currently tenant-scoped. The API returns successful data inside `{ "success": true, "data": ... }`; errors use `{ "success": false, "error": { "code", "message", "details?", "requestId" } }`. Session tokens are held in an HttpOnly, SameSite=Strict cookie and are never returned in JSON. Passwords are hashed using Node's scrypt implementation. The web app proxies `/api/v1/*` to the API so the browser can use a same-origin session cookie.
